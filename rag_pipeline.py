...
A comprehensive OOP-based pipeline for fetching and processing medical data
from multiple sources for RAG applications.

Usage:
    # Fetch and process everything
    python rag_pipeline.py fetch-all process-all
    ...
    python rag_pipeline.py --all              # Fetch and process everything
    python rag_pipeline.py --fetch-only       # Only fetch
    python rag_pipeline.py --process-only     # Only process
"""
...
class HTTPSession:
    ...
    def create():
        ...
        s.headers.update({"User-Agent": "OncologyImagingRAG/2.0"})
        return s
...
class TerminologyFetcher(BaseFetcher):
    ...
    def _create_loinc_instructions(loinc_dir: Path):
        ...
        note.write_text(
            ...
            "5. Run: python rag_pipeline.py fetch-terminology\n"
        )
...
class RAGPipeline:
    ...
    def run_full_pipeline(self):
        ...
        Logger.section("RUNNING FULL RAG PIPELINE")
        ...
        Logger.success("All RAG data ready for ingestion!")
...
def main():
    parser = argparse.ArgumentParser(
        description="Oncology Imaging RAG Data Pipeline",
...
Examples:
  ...
  python rag_pipeline.py process-pdfs
    ...
    # Simplified flags
    parser.add_argument('--all', action='store_true', help='Fetch and process everything')
    ...
    for cmd in args.commands:
        if cmd == 'fetch-all':
            ...
            pipeline.term_fetcher.fetch_loinc()
